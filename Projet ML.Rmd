...
Une prédiction est calculée sur la base de ce modèle linéaire dont Voici la matrice de confusion quand le seuil est à 50% :
```{r }
...
La matrice indique en ligne les valeurs prédites de la variable Cible et en colonne les vraies valeurs de cette variable. Les erreurs de prédictions sont donc les valeurs prédites "FALSE" alors que les valeurs réelles sont "TRUE" et les valeurs prédites "TRUE" alors que les vraies valeurs sont "TRUE".  
...
L'erreur de prédiction obtenue avec cette foret (ntree=5000 et mtry=4) est de `r rf6.err`%.   
...
**Avec un modèle de forêt aléatoire de ntree=5000 et mtry=4, on obtient un AUC est de `r rf6.auc`**
...
# erreur de classification
xgb.err2 = round(mean(as.numeric(pred.boost2 > 0.5) != ts_label)*100,2)
