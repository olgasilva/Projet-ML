...
**Après avoir tuné l'arbre, on obtient un AUC est de `r tree.auc`**
...
rf6.roc = performance(rf6.pred, measure="tpr", x.measure="fpr")
plot(roc, col="orange", lwd=2) 
lines(x=c(0, 1), y=c(0, 1), col="black", lwd=2)
```
...
```{r}
rf6.auc=performance(rf6.pred, measure = "auc")
...
Pour l'utiliser nous devons d'abord preparer les données, XGBoost fonctionne uniquement avec des vecteurs numériques. C'est-à-dire, nous devons transformer nos variables cibles, sex et city.type en nombre. 
...
train_numeric <- train_arbre %>% select_if(is.numeric) # select remaining numeric columns
...
```{r}
#using one hot encoding train 
sex2<-model.matrix(~sex-1,head(train_arbre))
city<-model.matrix(~city.type-1,head(train_arbre))
reg2<-model.matrix(~reg-1,head(train_arbre))
cat2<-model.matrix(~categorie-1,head(train_arbre))

train_numeric <- cbind(train_numeric,sex2,city,reg2,cat2)
train_matrix <- data.matrix(train_numeric)

#using one hot encoding test

sex3<-model.matrix(~sex-1,head(valid_arbre))
city3<-model.matrix(~city.type-1,head(valid_arbre))
reg3<-model.matrix(~reg-1,head(valid_arbre))
cat3<-model.matrix(~categorie-1,head(valid_arbre))

valid_numeric <- cbind(valid_numeric,sex3,city3,reg3,cat3)
...
This step isn't absolutely necessary, but it will help our model train move more quickly, and you'll need to to this if you ever want to train a model on multiple cores.
...
2-The number of training rounds. This just means the number of times we're going to improve our naive model by adding additional models. In the figure above going around the circle once = one boosting round.
...
err <- mean(as.numeric(pred > 0.5) != ts_label)
...
xgb.plot.multi.trees(feature_names = names(train_arbre), model = model_tuned)
...
importance_matrix <- xgb.importance( names(train_arbre), model = model_tuned)
...
```{r}
test <- chisq.test(train_arbre$reg, train_arbre$cible)
print(test)
```
