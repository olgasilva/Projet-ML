...
opt = round(optCut$optimalCutoff*100,2)
```

Nous pouvons optimiser cette prédiction, en trouvant le seuil optimal :`r opt` %. La matrice de confusion devient :
...
Comme dernière étape nous allons construire la courbe ROC et calculer l'AUC (area under the curve), qui sont des mesures de performance : la courbe ROC est la représentation des TPR (True positive rate, "Vrais positifs") en fonction des FPR (False positive rate, 'faux positifs), selon plusieurs seuils. La courbe de ROC doit être au dessus de la première bissectrice. Plus la courbe ROC de l'alogotithme tend vers 1 (100%) rapidement, meilleure est la qualité de la prédiction.  

L'AUC est l'aire sous la courbe ROC. Elle doit être comprise entre 0.5 et 1 (seuil AUC donnant la meilleur qualité de la prédiction).
...
auc.pred1=auc@y.values[[1]]
```


Ici on obtient un AUC de `r auc.pred1`.
...
print(rforest1)
#performance(rforest1, measure = "oob")
```

L'importance des variables explicatives selon le critère de Gini : plus le critère est bas, plus la valeur de la variable a une influence signicative sur la variable réponse.

```{r}
#importance des variables
#rforest$importance[order(rforest$importance[,1],decreasing=TRUE),]
#varImpPlot(rforest)
barplot(t(rforest1$importance))
#analyse graphique pour choisir ntree
plot(rforest1$err.rate[,1],type='l',xlab="nb arbres",ylab="erreur OOB")
...
rforest1.pred=predict(rforest1,newdata=valid_arbre)
table(rforest1.pred,valid_arbre$cible)

#sqrt((mean((as.integer(rforest.pred)-as.integer(valid_arbre$cible)^2)))

rf1.pred = prediction(as.numeric(rforest1.pred), as.numeric(valid_arbre$cible))
rf1.roc = performance(rf1.pred, measure="tpr", x.measure="fpr")
plot(roc, col="orange", lwd=2) 
lines(x=c(0, 1), y=c(0, 1), col="red", lwd=2)
```
...
```{r}
rf1.auc=performance(rf1.pred, measure = "auc")
rf1.auc=rf1.auc@y.values[[1]]
rf1.auc
```
...
```{r}
set.seed(1234)
rforest2=randomForest(cible~.,data=train_arbre,ntree = 5000,mtry = 2, na.action=na.omit)
print(rforest2)
```

L'importance des variables explicatives selon le critère de Gini : plus le critère est haut, plus la variable aura de l'influence pour distinguer le succes de l'echec de la campagne : ici l'age la variable la plus discriminante.

```{r}
#importance des variables
#rforest$importance[order(rforest$importance[,1],decreasing=TRUE),]
#varImpPlot(rforest)
barplot(t(rforest2$importance))
#analyse graphique pour choisir ntree
plot(rforest2$err.rate[,1],type='l',xlab="nb arbres",ylab="erreur OOB")
...
valid_arbre$cible[valid_arbre$cible == "TRUE"]=1
valid_arbre$cible=as.factor(valid_arbre$cible)
rforest2.pred=predict(rforest2,newdata=valid_arbre)
table(rforest2.pred,valid_arbre$cible)

#sqrt((mean((as.integer(rforest.pred)-as.integer(valid_arbre$cible)^2)))

rf2.pred = prediction(as.numeric(rforest2.pred), as.numeric(valid_arbre$cible))
rf2.roc = performance(rf2.pred, measure="tpr", x.measure="fpr")
plot(roc, col="orange", lwd=2) 
lines(x=c(0, 1), y=c(0, 1), col="red", lwd=2)
```
...
```{r}
rf2.auc=performance(rf2.pred, measure = "auc")
rf2.auc=rf2.auc@y.values[[1]]
rf2.auc
```
...
```{r}
#set.seed(12345)
#xgb.trcontrol=trainControl(method="cv",number=5,allowParallel=TRUE, verboseIter=FALSE, returnData=FALSE)
#xgb.Grid=expand.grid(nrounds=seq(50,500,by=50), #max_dept=6,eta=0.3,subsample=1,gmma=0,min_child_weight=1,colsample_bytree=1)
#xgb.model=train(cible~.,data=train_arbre,nmethod="xgbTree",trControl=xgb.trcontrol,tuneGrid=xgb.Grid)
