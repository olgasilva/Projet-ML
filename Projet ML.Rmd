...
Le modele qu'il a choisit avec le meilleure AIC est : cible ~ categorie + age + sex + REGION + city.type
...
```{r}
mean(abs((glm.pred1 > opt) - valid$cible), na.rm = T)
```
...
```{r}
New.tree <- rpart(cible~.,data=train_arbre,method="class",control = rpart.control(minsplit = 2, cp = 0))
```
