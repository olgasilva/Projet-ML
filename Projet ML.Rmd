...
learn_arbres = subset(learn, select=-c(insee.code,commune, department, latitude, longitude,reg))
...
Nous allons démarrer avec le CP = 0.001
```{r}
New.tree <- rpart(cible~.,data=train_arbre,method="class",control = rpart.control(minsplit = 2, cp = 0.001))
```
...
Avec plotcp, il vous montre également l'endroit optimal pour élaguer l'arbre. Un bon choix de CP pour l'élagage est souvent la valeur la plus à gauche pour laquelle la moyenne se situe en dessous de la ligne horizontale.
...
```{r}
plot(New.tree);text(New.tree, pretty=2)
