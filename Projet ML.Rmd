...
Dans le jeu test aussi, seule la variable "revenue" est manquante pour `r revtNA` valeurs, soit `r txrevtNA`% de valeurs manquantes.  
...
Une de première étapes d'analyse exploratoire de données est de vérifier que le jeu de données est équilibré. 
...
ggplot(learn,aes(cible)) + geom_bar(fill = "lightskyblue")+labs(title = "Répartition du succès/échec",y = "Nombre d'observations")

```

Cela veut dire que notre jeu de données et équilibré, donc, nous n'avons pas besoin d'appliquer des techniques de sous-échantillonnage ou SMOTE par exemple. 
...
Avec un AIC de (`r model2_aic`). Il semblerait dans ce choix que la variable "population" ne soit pas pertinent. 
...
```{r }
gml.tc=table(glm.pred1 > opt, valid$cible)
gml.tc
#prediction
...
tree.tc=table(tree.pred, valid_arbre$cible)
```
...
tree.tc2=table(tree.pred2, valid_arbre$cible)
tree.tc2
```
...
**La valeur optimale de CP est `r mincp`**, avec un **xerror de 0.12343**.  
Dans le graphique suivant on observe bien la diminution de xerror qui atteint son minimum et ensuite remonte à nouveau.
...
Nous allons obtenir l'importance des variables pour faire les splits de l'arbre de décision. L'importance des variables explicatives selon le critère de Gini signifie que : plus le critère est haut, plus la variable aura de l'influence pour distinguer le succès de l'échec de la campagne. 
...
L'erreur d'estimation se mesure ici par le taux d'erreur Out of Bag (OOB) : il s'agit de l'erreur d'estimation moyenne sur les échantillons d'apprentissage. 
...
Il semble être plus stable à 5000 arbres.  Nous comparons l' erreur OOB selon le nombre d'arbres : est la plus faible parmi les cas testés.
...
A 5000 arbres, l'erreur OOB est la plus faible.  

Nous fixons donc le nombre d'arbres à 5000. Essayons d'approcher maintenant le nombre optimum de variables explicatives testées à chaque séparation (mtry) . Nous testons mtry = 1 , 4 et 6 . 
...
#rf6.acc=ROCR::performance(rf6.pred, "acc")
rf6.perferr=ROCR::performance(rf6.pred, "err")
#rf6.err=rf6.preferr@y.values[[1]]
#rf6.err
```
...
xgb.plot.multi.trees(feature_names = names(train_numeric), model = model.boost )
```
...
#xgb.err1 = mean(as.numeric(pred.boost > 0.5) != ts_label)
#print(paste("test-error=", xgb.err1))
...
Uniquement avec des deux paramètres, notre erreur a baissé. Cependant nous avons d'autres paramètres qui peuvent être ajustés, comme :
...
Contrairement aux autres modèles, ici la catégorie et l'âge ne sont pas pertinents, on se base principalement sur le genre. 
...
----------------------- |-------------|----------------------|
 Regression logistique  | `auc.pred1` |`r misClasificError` %|     
 Arbre de décision      | `tree2.auc` |                      |             
 Forêt aléatoire        | `r rf6.auc` |                      |             
...
plot(prf3,col="pink", lwd=2, main="Courbe ROC")
par(new = T)
plot(rf6.roc, col="firebrick", lwd=2, main="Courbe ROC") 
par(new = T)
plot(xgb.roc, col="gold", lwd=2,main="Courbe ROC") 
legend("bottomright", legend=c("Régression linéaire", "Arbre décision", "Forêt aléatoire","Boosting"),
       col=c("orange", "pink", "firebrick","gold" ), lty=1, cex=1)
...
Nous choisissons donc de faire la prédiction à partir du modele de boosting.
...
write.csv(pred.boost3,"test_results.csv")
